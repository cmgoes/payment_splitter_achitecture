...
import {PlatformTokenDistributor} from "./PlatformTokenDistributor.sol";
...
contract FeeSharingSystem is ReentrancyGuard, Ownable {
    ...
    PlatformTokenDistributor public immutable platformTokenDistributor;
    ...
    /**
     * @notice Constructor
     * @param _platformToken address of the token staked (PlatformToken)
     * @param _rewardToken address of the reward token(USDT)
     * @param _tokenDistributor address of the Platform token distributor contract
     */
    constructor(
        ...
        platformTokenDistributor = PlatformTokenDistributor(_tokenDistributor);
    }

    /**
     * @notice Deposit staked tokens (and collect reward tokens if requested)
     * @param amount amount to deposit (in PlatformToken)
     * @param claimRewardToken whether to claim reward tokens
     * @dev There is a limit of 1 PlatformToken per deposit to prevent potential manipulation of current shares
     */
    function deposit(uint256 amount, bool claimRewardToken) external nonReentrant {
        require(amount >= PRECISION_FACTOR, "Deposit: Amount must be >= 1 PlatformToken");

        // Auto compounds for everyone
        platformTokenDistributor.harvestAndCompound();

        // Update reward for user
        _updateReward(msg.sender);

        // Retrieve total amount staked by this contract
        (uint256 totalAmountStaked, ) = platformTokenDistributor.userInfo(address(this));

        // Transfer Platform tokens to this address
        ...
        platformTokenDistributor.deposit(amount);
    ...
    /**
     * @notice Calculate pending rewards (USDT) for a user
    ...
    /**
     * @notice Calculate value of PlatformToken for a user given a number of shares owned
     * @param user address of the user
     */
    function calculateSharesValueInPlatform(address user) external view returns (uint256) {
        // Retrieve amount staked
        (uint256 totalAmountStaked, ) = platformTokenDistributor.userInfo(address(this));

        // Adjust for pending rewards
        totalAmountStaked += platformTokenDistributor.calculatePendingRewards(address(this));
    ...
    /**
     * @notice Calculate price of one share (in PlatformToken token)
     * Share price is expressed times 1e18
     */
    function calculateSharePriceInPlatform() external view returns (uint256) {
        (uint256 totalAmountStaked, ) = platformTokenDistributor.userInfo(address(this));
    ...
    function _withdraw(uint256 shares, bool claimRewardToken) internal {
        ...
        platformTokenDistributor.withdraw(currentAmount);
        ...
        // Transfer Platform tokens to sender
        platformToken.safeTransfer(msg.sender, currentAmount);
